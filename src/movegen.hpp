...
void generateCapturesSided(MoveList &moves, Position &pos) {
    ...
    // find the checks on the king and create capture mask
    U64 checkers = 0ULL;
...
template <bool white_to_move>
void generateQuietMovesSided(MoveList &moves, Position &pos) {
    // set up all bitboards for easy access according to friendly vs enemy with compiletime stuff
    constexpr const int side = static_cast<int>(white_to_move) ^ 1;
    constexpr const int enemy_side = side ^ 1;
...
template <bool white_to_move>
void generateLegalMoves(MoveList &moves, Position &pos) {
    generateCapturesSided<white_to_move>(moves, pos);
    generateQuietMovesSided<white_to_move>(moves, pos);
}
