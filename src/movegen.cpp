...
BitBoard getEnemyAttacks(Position &pos, Square sq) {
    ...
    attackers |= bishop_rays_from_sq & (pos.bitboards[getPieceID(BISHOP, other_side)] | pos.bitboards[getPieceID(QUEEN, other_side)]);
    BitBoard rook_rays_from_sq = getMagicRookAttack(sq, pos.bitboards[OCCUPANCY]);
    attackers |= rook_rays_from_sq & (pos.bitboards[getPieceID(ROOK, other_side)] | pos.bitboards[getPieceID(QUEEN, other_side)]);
...
BitBoard getCheckers(Position &pos, int king_index) {
    ...
    BitBoard checking_bishops = bishop_rays_from_king & (pos.bitboards[getPieceID(BISHOP, enemy_side)] | pos.bitboards[getPieceID(QUEEN, enemy_side)]);
    checkers |= checking_bishops;
    BitBoard rook_rays_from_king = getMagicRookAttack(king_index, pos.bitboards[OCCUPANCY]);
    BitBoard checking_rooks = rook_rays_from_king & (pos.bitboards[getPieceID(ROOK, enemy_side)] | pos.bitboards[getPieceID(QUEEN, enemy_side)]);
...
BitBoard getAllEnemyAttacks(Position &pos) {
    ...
    BitBoard enemy_attacks = pawnAttacksFromBitboard<getOtherSide(side)>(pos.bitboards[getPieceID(PAWN, enemy_side)]);
...
void generateMovesSided(MoveList &moves, Position &pos) {
    ...
    if (pos.movegen_data.generated_checkers) {
        ...
        if (checkers) {
            BitBoard bishop_rays_from_king = getMagicBishopAttack(king_index, pos.bitboards[OCCUPANCY]);
            BitBoard rook_rays_from_king = getMagicRookAttack(king_index, pos.bitboards[OCCUPANCY]);
            BitBoard checking_bishops = checkers & bishop_rays_from_king & (pos.bitboards[enemy_bishop] | pos.bitboards[enemy_queen]);
            BitBoard checking_rooks = checkers & rook_rays_from_king & (pos.bitboards[enemy_rook] | pos.bitboards[enemy_queen]);
        ...
        BitBoard bishop_rays_from_king = getMagicBishopAttack(king_index, pos.bitboards[OCCUPANCY]);
        BitBoard checking_bishops = bishop_rays_from_king & (pos.bitboards[enemy_bishop] | pos.bitboards[enemy_queen]);
        checkers |= checking_bishops;
        BitBoard rook_rays_from_king = getMagicRookAttack(king_index, pos.bitboards[OCCUPANCY]);
        BitBoard checking_rooks = rook_rays_from_king & (pos.bitboards[enemy_rook] | pos.bitboards[enemy_queen]);
        checkers |= checking_rooks;
        // king check detection needed for pseudolegal moves
    ...
    // pinned on diagonals
    BitBoard blockers = getMagicBishopAttack(king_index, pos.bitboards[OCCUPANCY]) & pos.bitboards[OCCUPANCY];
    ...
    BitBoard pin_rays = bishopAttacksFromBitboard(pinners, pos.bitboards[OCCUPANCY] & ~blockers) & xray;
    ...
    BitBoard pinned_bishops_and_queens = pinned_pieces & (pos.bitboards[friendly_bishop] | pos.bitboards[friendly_queen]);
    ...
    BitBoard pinned_rooks_and_queens = pinned_on_rank_and_file & (pos.bitboards[friendly_rook] | pos.bitboards[friendly_queen]);
            ...
            if ((num_checks == 0) || (pawn_attacks[enemy_side][piece_index] & pos.bitboards[friendly_king])) {
                ...
                while (en_passant_attackers) {
                    ...
                    if (pos.bitboards[friendly_king] & ep_rank) { 
                        ...
                        BitBoard ep_pin_rays = getMagicRookAttack(king_index, pos.bitboards[OCCUPANCY] & ~setBit(start_square) & ~setBit(piece_index));
    ...
    BitBoard bishops = (pos.bitboards[friendly_bishop] | pos.bitboards[friendly_queen]) & ~pinned_pieces;
    ...
    BitBoard rooks = (pos.bitboards[friendly_rook] | pos.bitboards[friendly_queen]) & ~pinned_pieces;
